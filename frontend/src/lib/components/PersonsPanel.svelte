...
<div class="max-w-lg mx-auto p-6 space-y-6">
	...
	<div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-5">
		<p class="text-xs text-stone-300 uppercase tracking-wide mb-4">Who's tasting coffee</p>

		{#if tasters.length === 0}
			<div class="text-center py-6">
				<Icons icon="cup" size={32} className="mx-auto text-stone-200 mb-2" />
				<p class="text-sm text-stone-300">No tasters yet — add yourself!</p>
			</div>
		{:else}
		...
		<div class="flex gap-2">
			<input type="text" bind:value={newName} placeholder="Add person..."
