...
<div class="max-w-lg mx-auto p-6 space-y-6">
	...
	<div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-5">
		...
		{#each grinders as g}
			<div class="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-stone-50">
				<div class="flex items-center gap-2">
					<span class="text-sm text-stone-700">{g.name}</span>
					{#if g.model}<span class="text-xs text-stone-300">{g.model}</span>{/if}
					{#if g.is_default}<span class="px-1.5 py-0.5 text-[10px] bg-amber-100 text-amber-700 rounded">default</span>{/if}
				</div>
				<div class="flex gap-1">
					{#if !g.is_default}
						<button onclick={() => setDefault(g.id)} class="px-1.5 py-0.5 text-[10px] text-stone-300 hover:text-amber-600">set default</button>
					{/if}
					<button onclick={() => deleteEquipment(g.id)} class="p-1 text-stone-200 hover:text-red-400" title={$t('detail.delete')}>
		...
		<div class="flex gap-2 mt-2">
			<input type="text" bind:value={newGrinderName} placeholder="Add grinder..."
				class="flex-1 px-3 py-1.5 rounded-lg border border-stone-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50"
			...
			<button onclick={() => addEquipment('grinder', newGrinderName)} disabled={!newGrinderName.trim()}
				class="px-3 py-1.5 bg-amber-700 text-white rounded-lg text-xs hover:bg-amber-800 disabled:opacity-50">{$t('persons.add')}</button>
		</div>
	</div>
		...
		{#each machines as m}
			<div class="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-stone-50">
				<div class="flex items-center gap-2">
					...
					{#if m.is_default}<span class="px-1.5 py-0.5 text-[10px] bg-amber-100 text-amber-700 rounded">default</span>{/if}
				</div>
				<div class="flex gap-1">
					{#if !m.is_default}
						<button onclick={() => setDefault(m.id)} class="px-1.5 py-0.5 text-[10px] text-stone-300 hover:text-amber-600">set default</button>
					{/if}
					<button onclick={() => deleteEquipment(m.id)} class="p-1 text-stone-200 hover:text-red-400" title={$t('detail.delete')}>
						<Icons icon="delete" size={14} />
					</button>
				</div>
			</div>
	...
	<div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-5">
		...
		{#each brewMethods as m}
			<div class="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-stone-50">
				...
				<button onclick={() => deleteMethod(m.id)} class="p-1 text-stone-200 hover:text-red-400" title={$t('detail.delete')}>
					<Icons icon="delete" size={14} />
				</button>
			</div>
		{/each}
	...
	<div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-5">
		...
		<div class="flex flex-wrap gap-2 mb-3">
			{#each basketSizes as bs}
				<div class="flex items-center gap-1 px-3 py-2 bg-stone-50 rounded-lg {bs.is_default ? 'ring-2 ring-amber-300' : ''}">
		...
		<div class="flex gap-2">
			<input type="number" bind:value={newBasketGrams} placeholder="grams"
				class="w-24 px-3 py-1.5 rounded-lg border border-stone-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/50"
